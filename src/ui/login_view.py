...
class LoginView:
    ...
    def _init_help_message(self, text1="enter username and password to log in", text2="account will be created if it doesn't exist"):
        ...
        self.account_created_label1.grid(row=0, column=0, columnspan=3, sticky=constants.W)
        self.account_created_label2.grid(row=1, column=0, columnspan=3, sticky=constants.W)
    ...
    def _handle_login(self):
        ...
        try:
            ...
            try:
                ...
                self._init_help_message(f"created account with user {username}", "click login again to log in")
            except UserExistsError:
                self._init_help_message("a user with that name exists,", "but the password is incorrect")
